...
function App() {
  ...
  useEffect(() => {
    loadData()
      .then(setData)
      .catch(() => showToast("Failed to load data.json", true));
  }, []);
    ...
    (action) => {
      setData((prev) => {
        ...
        switch (action.type) {
          ...
          case "DELETE": {
            next[action.list] = next[action.list].filter(
              (x) => x.id !== action.id,
            );
          ...
          case "SAVE_ITEM": {
            const listKey =
              action.modalType === "service"
                ? "services"
                : action.modalType === "tutorial"
                  ? "tutorials"
                  : "bookmarks";
            if (action.isEdit) {
              const idx = next[listKey].findIndex(
          ...
          case "EXPORT": {
            const blob = new Blob([JSON.stringify(prev, null, 2)], {
          ...
          case "RESET": {
            if (
              !confirm(
                "Reset everything to defaults? All customizations will be lost.",
              )
            )
              return prev;
            (async () => {
  ...
  // --- Derived state ---
  const categories = [
    ...new Set(data.services.map((s) => s.cat).filter(Boolean)),
  ].sort();
  ...
  const filteredServices = data.services.filter((s) => {
    ...
    const matchesSearch =
      !q || (s.name + " " + s.tags + " " + s.desc).toLowerCase().includes(q);
    return matchesCat && matchesSearch;
  });

  const filteredTutorials = data.tutorials.filter(
    (t) =>
  ...
  return html`
    <${AppContext.Provider} value=${ctx}>
      ...
      <${Section}
        ...
        visible=${filteredServices.length > 0 || editMode}
      >
        <div class="grid">
          ...
          <div
            class="add-card"
            onClick=${() =>
              dispatch({ type: "OPEN_MODAL", modalType: "service" })}
          >
      ...
      <${Section}
        ...
        visible=${filteredTutorials.length > 0 || editMode}
      >
        <div class="grid">
          ${filteredTutorials.map(
            (t) => html` <${TutorialCard} key=${t.id} tutorial=${t} /> `,
          )}
          <div
            class="add-card"
            onClick=${() =>
              dispatch({ type: "OPEN_MODAL", modalType: "tutorial" })}
          >
      ...
      <${Section}
        title="Quick Links"
        visible=${filteredBookmarks.length > 0 || editMode}
      >
        <div class="bookmark-group">
          ${filteredBookmarks.map(
            (b) => html` <${BookmarkItem} key=${b.id} bookmark=${b} /> `,
          )}
          <span
            class="add-bookmark"
            onClick=${() =>
              dispatch({ type: "OPEN_MODAL", modalType: "bookmark" })}
          >
      ...
      <div class="bottom-row">
        <${Section} title="Scratchpad" noMargin>
          <${Notepad}
            value=${data.notes}
            field="notes"
            placeholder="Quick notes, IPs, TODOs..."
            showFooter
          />
        <//>
...
Subnet: 192.168.1.0/24"
      ...
      <footer>
        <kbd>/</kbd> search · <kbd>Esc</kbd> clear · <kbd>T</kbd> theme ·
        <kbd>E</kbd> edit mode
      </footer>
    <//>
  `;
