...
export function ServiceCard({ service, isFav, status }) {
  ...
  if (service.favicon) {
    iconContent = html`
      ...
      <span style="display:none"><${DynamicIcon} value=${service.icon} size=${20} /></span>
    `;
  } else {
    iconContent = html`<${DynamicIcon} value=${service.icon} size=${20} />`;
  }

  const statusClass = status === true ? 'up' : status === false ? 'down' : 'unknown';

  return html`
    <a ref=${ref} href=${service.url} target="_blank"
      ...
      ${!editMode && html`
        <button class="fav-btn ${isFav ? 'is-fav' : ''}"
                onClick=${(e) => { e.preventDefault(); e.stopPropagation(); dispatch({ type: 'TOGGLE_FAV', id: service.id }); }}>
          <${StarIcon} size=${12} />
        </button>
      `}
      ...
      ${editMode && html`
        <button class="del-btn"
                onClick=${(e) => { e.preventDefault(); e.stopPropagation(); if (confirm('Remove this service?')) dispatch({ type: 'DELETE', list: 'services', id: service.id }); }}>
          <${XIcon} size=${12} />
        </button>
        <button class="edit-btn"
                onClick=${(e) => { e.preventDefault(); e.stopPropagation(); dispatch({ type: 'OPEN_MODAL', modalType: 'service', editId: service.id }); }}>
          <${PencilIcon} size=${12} />
        </button>
      `}
    </a>
