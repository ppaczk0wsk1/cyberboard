...
export function IconPicker({ value, onChange }) {
  ...
  const displayEmojis = isSearching
    ? filtered
    : activeCat
      ? EMOJI_DATA[activeCat].map(([emoji, keywords]) => ({ emoji, keywords, cat: activeCat }))
      : null;

  return html`
    <div class="icon-picker-container">
      <input class="icon-search" type="text" value=${query}
             onInput=${(e) => { setQuery(e.target.value); setActiveCat(null); }}
             placeholder="Search emojis... (e.g. server, lock, cloud, fire)" />

      ${!isSearching && html`
        <div class="icon-picker-tabs">
          ${CATEGORY_NAMES.map(cat => html`
            <button key=${cat}
      ...
      ${displayEmojis !== null && html`
        <div class="icon-grid">
          ${displayEmojis.map(({ emoji }) => html`
            <div key=${emoji + Math.random()}
                 class="icon-grid-item emoji ${value === emoji ? 'selected' : ''}"
                 onClick=${() => onChange(emoji)}
                 title=${emoji}>
              ${emoji}
            </div>
          `)}
          ${displayEmojis.length === 0 && html`
            <div class="icon-grid-empty">No emojis match "${query}"</div>
          `}
        </div>
      `}

      ${!isSearching && !activeCat && html`
        <div class="icon-grid-hint">Pick a category or search above</div>
      `}
    </div>
  `;
}
